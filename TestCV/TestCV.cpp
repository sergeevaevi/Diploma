...
void InsertOldPlaces(IplImage* orig_img)   {
	for (int i = 0; i < old.size(); i++) {
		Mat subImg = imread(to_string(i) + "_old.jpg");
		//wana see this?
		//transform
		auto src_img = TransformMatToImg(subImg);
		//start insert
		cvSetImageROI(orig_img, old[i]);
		// обнулим изображение
		cvZero(orig_img);
...
void InsertNewPlaces(IplImage* orig_img, vector<Rect> objects) {
	for (int i = 0; i < objects.size(); i++) {
		...
		cvSetImageROI(orig_img, objects[i]);
		// обнулим изображение
		cvZero(orig_img);
		//// копируем изображение
		cvCopy(src_img, orig_img);
